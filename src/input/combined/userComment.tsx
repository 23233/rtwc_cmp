...
import { inputAreaParams, inputAreaRef, InputAreaView, StarRate, starRateParams } from '@rtwc/cmp';
import { Btn, Cwal, EmojiItem, EmojiView, fastImgsPreviewOpen, Lmg, Spin, Toast } from '@rtwc/cmp';
...
const UserCommentInput: React.FC<userCommentInputParams> = ({
  ...
  const onImgClick = (now: string) => {
    const previewData = fileList.map((d) => d.origin);
    fastImgsPreviewOpen(previewData, now);
  };

  const onImgRemove = async (item: fileUploadResp) => {
    const resp = await Cwal.fire({
      title: '确认删除此图片吗?',
  ...
  const onLinkRemove = async (item: inputLinkParams) => {
    const resp = await Cwal.fire({
      title: '确认删除此链接吗?',
      confirmButtonText: '确认',
      showCancelButton: true,
